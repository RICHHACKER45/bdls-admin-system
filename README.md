# A Basic Admin-Dashboard for Local Barangays

This repository contains a **Basic Admin-Dashboard** developed for barangay service management. The system features an automated database initializer, real-time queue monitoring, and advanced security hardening.
...
* **AJAX-Powered UI:** Seamless data fetching and UI updates using Fetch API for a "Single Page Application" feel.
...
* **Clean URLs:** Extensionless routing (e.g., `/login` instead of `/login.php`) to obscure the technology stack.
...
3. The system will detect if `bdls_db` is missing and will execute `database/init.sql` automatically upon your first visit.
